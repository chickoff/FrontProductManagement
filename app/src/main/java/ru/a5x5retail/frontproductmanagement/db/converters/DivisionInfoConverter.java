...
public class DivisionInfoConverter implements IDbConvertible<DivisionInfo> {
    @Override
    public boolean Convert(ResultSet resultSet, DivisionInfo obj) throws SQLException {
        obj.guid = resultSet.getObject("GUID").toString();
        obj.nameLong = resultSet.getString("NameLong");
        obj.inn = resultSet.getString("INN");
        obj.kpp = resultSet.getString("KPP");
        obj.postalAddress = resultSet.getString("PostalAddress");
        return false;
    }
