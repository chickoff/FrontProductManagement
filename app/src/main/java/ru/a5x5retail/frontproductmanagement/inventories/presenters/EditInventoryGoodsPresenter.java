...
import ru.a5x5retail.frontproductmanagement.db.query.read.GetInventoryCodeQuery;
...
import ru.a5x5retail.frontproductmanagement.db.query.update.EditInventoryGoodsQuery;
...
@InjectViewState
public class EditInventoryGoodsPresenter extends BasePresenter<IEditInventoryGoodsView> {
   ...
   // public InventoryUoW inventoryUoW;
    ...
    private void load() {
        ...
        final GetInventoryCodeQuery q = new GetInventoryCodeQuery(model.getSelectedInventoryList().inventoryGuid);
        registerAwaitEvents(q);
        q.addOnSuccessfulListener(new DataBaseQuery.OnSuccessfulListener() {
            @Override
            public void onSuccessful() {
                inventoryCodes = q.getList();
                getViewState().updateUi();
            }
        });
    ...
    public void updatePosition(InventoryCode ic, BigDecimal nq) {
        StringBuilder sb = new StringBuilder();
        sb
        .append("<root>")
            .append("<sku>")
                .append("<code>").append(ic.code).append("</code>")
                .append("<val>").append(nq).append("</val>")
            .append("</sku>")
        .append("</root>");

        EditInventoryGoodsQuery q = new EditInventoryGoodsQuery(
                model.getSelectedInventoryList().inventoryGuid,
                ProjectMap.getCurrentTypeDoc().getTypeOfDocument().getIndex(),
